      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          {Object.entries(skillCategories).map(([key, category], index) => (
            <div
              key={key}
              className={`bg-slate-800/60 backdrop-blur-sm rounded-xl md:rounded-lg p-5 md:p-6 border border-cyan-400/20 hover:border-cyan-400/40 transition-all duration-300 md:hover:scale-105 shadow-lg hover:shadow-cyan-400/10 ${
            ...
            >
              ...
              <h3 className="text-lg md:text-xl font-bold text-white mb-3 md:mb-4 tracking-tight">{category.title}</h3>
        ...
        <div 
          ref={competenciesRef}
          className={`bg-slate-800/60 backdrop-blur-sm rounded-2xl p-6 md:p-8 border border-cyan-400/20 transition-all duration-1000 delay-700 shadow-lg ${
            competenciesVisible ? 'animate-fade-in opacity-100' : 'opacity-0 translate-y-10'
          }`}
        >
          <h3 className="text-xl md:text-2xl font-bold text-white mb-6 md:mb-8 text-center tracking-tight">Core Competencies</h3>
          <div className="flex flex-wrap justify-center gap-3 md:gap-4">
            {coreCompetencies.map((competency, index) => (
              <span
                key={index}
                className="px-4 py-2 bg-slate-700/50 text-cyan-400 rounded-full border border-cyan-400/30 hover:bg-cyan-400/10 transition-colors duration-200 text-sm md:text-base shadow-md"
              >
                {competency}
              </span>
            ))}
